...
class ConcurrentStrategySpec extends munit.FunSuite {
  ...
  private implicit val concurrentIO: Concurrent[IO] = IO.ioConcurrentEffect

  test("CE2 Concurrent matches Parallel (parallel call: Workload.scala:23, concurrent call: Workload.scala:29)") {
  ...
  test("CE2 Concurrent uses start/join path (call site: Workload.scala:29)") {
